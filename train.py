...
from ptb import PTB
from poems import PoetryDataset
...
def main(args):
    ...
    if args.tensorboard_logging:
        writer = SummaryWriter(os.path.join(
            args.logdir, expierment_name(args, ts)))
    ...
    NLL = torch.nn.NLLLoss(
        size_average=False, ignore_index=datasets['train'].pad_idx)
    ...
    for epoch in range(args.epochs):
        ...
        for split in splits:
            ...
            for iteration, batch in enumerate(data_loader):
                ...
                # Forward pass
                logp, mean, logv, z = model(batch['input'], batch['length'], condition=batch['category'].float())
                ...
                if args.tensorboard_logging:
                    writer.add_scalar(
                        "%s/ELBO" % split.upper(), loss.data[0], epoch*len(data_loader) + iteration)
                    writer.add_scalar(
                        "%s/NLL Loss" % split.upper(), NLL_loss.data[0]/batch_size, epoch*len(data_loader) + iteration)
                    writer.add_scalar(
                        "%s/KL Loss" % split.upper(), KL_loss.data[0]/batch_size, epoch*len(data_loader) + iteration)
                    writer.add_scalar(
                        "%s/KL Weight" % split.upper(), KL_weight, epoch*len(data_loader) + iteration)
                ...
                if split == 'valid':
                    ...
                    tracker['target_sents'] += idx2word(
                        batch['target'].data, i2w=datasets['train'].get_i2w(), pad_idx=datasets['train'].pad_idx)
                    tracker['z'] = torch.cat((tracker['z'], z.data), dim=0)
                    # Calculate BLEU score
                    decoded = torch.argmax(logp, dim=-1)
                    
                    for i in range(decoded.shape[0]):
                        decoded_poem = idx2word([decoded[i]], i2w=datasets['train'].get_i2w(), pad_idx=datasets['train'].pad_idx)[0]
                        original_poem = idx2word([batch['target'].data[i]], i2w=datasets['train'].get_i2w(), pad_idx=datasets['train'].pad_idx)[0]
                        total_BLEU_score += calculate_bleu_scores(original_poem, decoded_poem)
                    
            print("%s Epoch %02d/%i, Mean ELBO %9.4f" %
                  (split.upper(), epoch, args.epochs, torch.mean(tracker['ELBO'])))
            if split == 'valid':
                print("Average BLEU {}".format(total_BLEU_score / decoded.shape[0]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default='poem_data')
    ...
    parser.add_argument('-x0', '--x0', type=int, default=1000)
